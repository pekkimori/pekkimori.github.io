...
modDatetime: 2025-08-11T01:11:13Z
...
- Claiming Earth is reasonably round is science, because it takes a look and see that it is flat to falsify it; but after the look, we saw it is very round, this is falsifiable and true.
...
We need methodological rigor, transparency, and honesty to make sure the argument makes sense and is true, given that the experiment or observation results are correct, giving the study validity.

Along with this, we must make sure the premises we are considering are also true and that the results yielded from the experiment or observation are actually correct, giving the study soundness (or accuracy/correctness).

It means nothing if a measurement validates something once and only once; it should be consistent for every measurement; we need to make sure it was not a lucky result to be considered reliable evidence. That's also why people must be able to reproduce the study (i.e., do the EXACT same thing and get the EXACT same results), giving the study reliability.
...
https://www.youtube.com/watch?v=YAgJ9XugGBo" target="_blank">spinning fish</a>, which often make proper measurements inaccurate. So as you can see, humans are so complex that it is tremendously difficult to indicate the cause or correlation of one action and an event.
